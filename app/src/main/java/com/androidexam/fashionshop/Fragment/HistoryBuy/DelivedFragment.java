...
public class DelivedFragment extends Fragment {
    ...
    public View onCreateView(LayoutInflater inflater, ViewGroup container,
        ...
        View view= inflater.inflate(R.layout.fragment_delived, container, false);
        SharedPreferences preferences = getContext().getSharedPreferences("ID_USER", Context.MODE_PRIVATE);
        userId = preferences.getInt("user_id", -1);
        recyclerView = view.findViewById(R.id.rcv_delivered);
    ...
    public void ViewOrderItemRated() {
        ...
        ApiService.productServiceWithToken.getOrder(userId,"DELIVERED").enqueue(new Callback<ResponseGetOrdered>() {
            @Override
            public void onResponse(Call<ResponseGetOrdered> call, Response<ResponseGetOrdered> response) {
                if (response.isSuccessful()) {
                    ...
                    OrderedItemAdapter orderedItemAdapter = new OrderedItemAdapter(getContext(), listOrdered,requireActivity().getSupportFragmentManager(),"delivered");
                    LinearLayoutManager layoutManager = new LinearLayoutManager(requireContext());
                    recyclerView.setLayoutManager(layoutManager);
