...
public class CancelFragment extends Fragment {
    ...
    public void ViewOrderItemRated() {
        ...
        ApiService.productServiceWithToken.getOrder(userId,"CANCELLED").enqueue(new Callback<ResponseGetOrdered>() {
            @Override
            public void onResponse(Call<ResponseGetOrdered> call, Response<ResponseGetOrdered> response) {
                if (response.isSuccessful()) {
                    ...
                    OrderedItemAdapter orderedItemAdapter = new OrderedItemAdapter(getContext(), listOrdered,requireActivity().getSupportFragmentManager(),"cancel");
                    LinearLayoutManager layoutManager = new LinearLayoutManager(requireContext());
                    recyclerView.setLayoutManager(layoutManager);
