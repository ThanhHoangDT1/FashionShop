...
public class ConfirmedFragment extends Fragment {
    ...
    public View onCreateView(LayoutInflater inflater, ViewGroup container,
        ...
        recyclerView = view.findViewById(R.id.rcv_conform);
    ...
    public void ViewOrderItemRated() {
        ...
        ApiService.productServiceWithToken.getOrder(userId,"CONFIRMED").enqueue(new Callback<ResponseGetOrdered>() {
            @Override
            public void onResponse(Call<ResponseGetOrdered> call, Response<ResponseGetOrdered> response) {
                if (response.isSuccessful()) {
                    ...
                    OrderedItemAdapter orderedItemAdapter = new OrderedItemAdapter(getContext(), listOrdered,requireActivity().getSupportFragmentManager(),"confirm");
                    LinearLayoutManager layoutManager = new LinearLayoutManager(requireContext());
                    recyclerView.setLayoutManager(layoutManager);
